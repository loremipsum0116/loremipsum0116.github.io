...
설명:
<br>
  * 바깥 `for`문의 변수 `i`는 현재 곱셈의 첫 번째 숫자를 나타냅니다 (1, 2, 3).<br>
  * 안쪽 `for`문의 변수 `j`는 현재 곱셈의 두 번째 숫자를 나타냅니다 (1, 2, 3).<br>
  * 따라서 `i * j`는 이 두 숫자의 곱셈 결과를 의미하며, 이를 `System.out.println`을 통해 출력하게 됩니다.
