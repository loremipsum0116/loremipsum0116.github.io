...
| 특징 / 구분       | 서블릿 사용 시 (JSP/Servlet, Spring MVC)                               | 서블릿 미사용 시 (정적 HTML, JS 프레임워크 기반 SPA)             |
| :---------------- | :--------------------------------------------------------------------- | :--------------------------------------------------------------- |
| **개념/기술 스택** | - **서버 사이드 기술 (자바 기반):** Java Servlet, JSP<br> - **프레임워크:** Spring MVC (내부적으로 서블릿 기반) | - **클라이언트 사이드 기술:** HTML, CSS, JavaScript<br> - **프레임워크:** React, Angular, Vue.js (SPA)<br> - **백엔드 (API):** Node.js, Python/Django, Ruby on Rails, Spring Boot 등 (서블릿 아님) |
| **역할** | - **동적인 웹 페이지 생성:** 클라이언트 요청에 따라 서버에서 동적으로 HTML 생성 및 반환<br> - **비즈니스 로직 처리:** 데이터베이스 연동, 인증/인가, 복잡한 계산 등 | - **정적 파일 제공:** 웹 서버가 HTML, CSS, JS 파일을 클라이언트에 제공<br> - **클라이언트 렌더링:** 브라우저에서 JavaScript를 통해 동적으로 UI 렌더링 및 데이터 표시<br> - **백엔드 통신:** RESTful API를 통해 백엔드와 데이터 주고받기 |
| **초기 로딩 방식** | - **서버 사이드 렌더링 (SSR):** 각 페이지 요청 시 서버에서 완전한 HTML 페이지를 생성하여 전송 | - **클라이언트 사이드 렌더링 (CSR) 또는 SPA:** 최초 로딩 시 정적 HTML/CSS/JS 번들 전송, 이후 필요한 데이터만 API로 가져와 클라이언트에서 렌더링 |
| **데이터 처리** | - 서버에서 직접 DB 연동 및 비즈니스 로직 수행 후 결과 페이지 생성   | - 클라이언트(JavaScript)에서 API를 호출하여 데이터 수신 후 화면에 표시 |
| **페이지 전환** | - **페이지 새로고침:** 다른 페이지로 이동 시 브라우저가 전체 페이지를 다시 로드 | - **부분 새로고침:** (SPA의 경우) 필요한 부분만 동적으로 업데이트, 전체 페이지 새로고침 없음 |
| **개발 복잡성** | - **초기 설정 복잡성:** WAS 설정, `web.xml` 등<br> - **HTML/Java 혼재 가능성:** 유지보수 어려움 (JSP)<br> - **프레임워크 사용 시:** Spring MVC 등은 학습 곡선 존재 | - **백엔드와 프론트엔드 분리:** 개발 환경 및 빌드 파이프라인 구성 필요<br> - **JavaScript 프레임워크 학습:** 각 프레임워크의 개념 및 도구 사용법 숙지 필요<br> - **CORS, 라우팅, 상태 관리 등 개념 이해 필요** |
| **성능/사용자 경험** | - **페이지 로딩 시간:** 각 페이지별 서버 응답 시간 발생<br> - **SEO 유리:** 초기 HTML에 모든 콘텐츠 포함 | - **초기 로딩 시간:** 번들 크기에 따라 지연될 수 있음<br> - **이후 빠른 전환:** SPA의 경우 부드러운 페이지 전환<br> - **SEO 불리:** (SSR 없이 CSR만 사용 시) 초기 HTML에 콘텐츠가 없어 SEO에 불리할 수 있음 (SSR/Prerendering으로 해결 가능) |
| **적합한 상황** | - **SEO가 매우 중요한 사이트:** 블로그, 뉴스 등<br> - **초기 개발 속도가 중요한 소규모 프로젝트**<br> - **레거시 시스템과의 연동**<br> - **클라이언트 자원이 제한적인 환경** | - **동적이고 상호작용이 많은 웹 앱:** 대시보드, 관리자 페이지, 쇼핑몰, SaaS 등<br> - **모바일 앱과 API를 공유하는 경우**<br> - **빠른 사용자 경험이 중요한 경우**<br> - **프론트엔드/백엔드 개발 조직이 분리된 경우** |
| **현업 트렌드** | - Spring Boot + Thymeleaf/Mustache와 같은 서버 사이드 템플릿 엔진 조합으로 대체되거나, REST API 백엔드로 역할 축소 | - Spring Boot (백엔드) + React/Angular/Vue.js (프론트엔드) 조합이 가장 일반적인 현대 웹 개발 방식 |
