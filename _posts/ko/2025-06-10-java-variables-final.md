...
> **앞서 언급했듯, 지역 변수는 아래 값으로 자동 초기화되지 않으므로 직접 초기화가 필요합니다.**
...
- **camelCase** 명명 규칙을 따르는 것이 가독성과 유지보수에 도움됨 (camelCase란, `helloWorld`, `mySweetHome` 과 같이 맨 앞에 소문자가 오며 맞붙는 다른 단어의 첫 글자를 대문자로 쓰는 명명 규칙을 말합니다.)
