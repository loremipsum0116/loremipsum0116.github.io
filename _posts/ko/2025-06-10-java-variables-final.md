...
>
> 아마 학창 시절 수학 시간에 만났던 '미지수 x'처럼, **'아직 정해지지 않은 값'**을 먼저 떠올리는 분도 계실 거예요. 하지만 자바(Java)를 비롯한 대부분의 프로그래밍 언어에서 변수는 조금 더 실용적이고 구체적인 의미를 가집니다. 바로 **"데이터를 저장하기 위해 컴퓨터 메모리 공간에 붙여진 이름"**입니다.
>
> 마치 여러분이 책상 서랍에 '펜', '노트', '충전기'와 같은 이름을 붙여두고 필요한 물건을 쉽게 찾고 정리하는 것처럼, 프로그래밍에서 변수는 우리가 다루는 데이터를 효율적으로 관리할 수 있도록 돕는 핵심 도구입니다.
>
...
>
> **클래스 기반 변수(인스턴스 변수, 클래스 변수)**에 대한 더 깊이 있는 내용은 자바의 꽃이라고 할 수 있는 **객체 지향 프로그래밍(OOP)**을 다루는 다음 글에서 상세히 설명해 드릴 예정입니다. 그때까지는 '변수가 이런 종류로 나뉘는구나' 정도로 이해하셔도 충분합니다!
...
## 📌 지역 변수는 반드시 초기화! 초기화하지 않으면 컴파일 에러가 발생합니다
...
보셨나요? 지역 변수는 값이 확정되어야만 사용할 수 있습니다. 개발자가 명시적으로 값을 지정해주는 것은 선택이 아니라 필수입니다. 이 작은 규칙 하나가 여러분의 프로그램을 더욱 견고하게 만들어 줄 거예요!
...
변수 이름을 잘 짓는 것은 마치 좋은 글을 쓰는 것과 같습니다. 여러분의 코드가 '읽히는' 언어가 되도록 노력해 보세요!
...
변수는 여러분이 자바 개발자로서 첫걸음을 내딛는 데 가장 기본이 되면서도 핵심적인 개념입니다. 이 기초를 튼튼히 다진다면, 앞으로 만나게 될 더욱 복잡한 개념들도 훨씬 수월하게 익힐 수 있을 거예요.
